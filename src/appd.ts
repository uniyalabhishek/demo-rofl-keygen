...
/**
 * generateKey
 * Calls appd REST: POST /rofl/v1/keys/generate over the UNIX socket
 * /run/rofl-appd.sock. Returns a 0x-prefixed hex secret key string.
 */
        ...
        res.on("end", () => {
          try {
            ...
            if (!hex || typeof hex !== "string") {
              return reject(new Error(`Bad response: ${body}`));
            }
            const sk = hex.startsWith("0x") ? hex : `0x${hex}`;
            resolve(sk);
          } catch (e) {
            reject(e);
          }
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (!/^rofl1[0-9a-z]+$/.test(id)) {
            return reject(new Error(`Bad app id: ${body}`));
          }
          resolve(id);
        });
      }
...
/**
 * getEvmSecretKey
 * Wrapper that first tries ROFL keygen. If not available and LOCAL_DEV_SK is
 * set (and ALLOW_LOCAL_DEV=true), returns that for local development only.
 */
